    ...
    useEffect(() => {
        const fetchStudents = async () => {
            try {
                ...
                const filteredStudents = allStudents.filter(student => student.institution.toString() === storedInstitutionId);
                setStudents(filteredStudents);
            } catch (error) {
                console.error('Error al cargar los estudiantes:', error);
            }
    ...
    return (
        <div className="chat-profesor-container">
            ...
            <div className="chat-bubbles-container">
                {students.map(student => (
                    <div key={student.id} className="student-bubble" onClick={() => {
                        setSelectedStudent(student.id);
                        fetchMessages(); 
                    }}>
                        <img src={student.imagen_url} alt={`${student.name} profile`} className="student-photo" />
                        <p>{student.name}</p>
                    </div>
                ))}
            </div>
                ...
                <div className="messages-container">
                    {filteredMessages.length > 0 ? (
                        filteredMessages.map((msg, index) => (
                            <div
                                key={index}
                                className={`message ${msg.staff.toString() === storedStaffId ? 'sent' : 'received'}`}
                            >
                                <strong>{msg.name}:</strong> {msg.message}
                            </div>
                        ))
